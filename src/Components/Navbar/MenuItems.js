            ...
            {   
                items.submenu && (items.submenu.length || Object.values(items.submenu).length) ? ( 
                    <>
                        {
                        items.url === '#'?
                        <span className="menu-item-link" onClick={handleClick} >{items.title}</span>:
                        <NavLink className="menu-item-link" to={items.url} onClick={(event)=>menuOpenToggle(!menuOpen)} >{items.title}</NavLink>
                        }
