...
const Testimonial = (props) => {
    return (
        <div className="item-wrapper singleFeedback isRadius16 p-30 bgClrDarkLight d-flex flex-column justify-content-between">
            <div className="reviewerContent">
                <div className="logos mb-4">
                  <LazyImage className="testimonial-company" src={props?.data?.meta._mosacademy_testimonial_company_logo} alt={[props?.data?.meta._mosacademy_testimonial_name, '- Conpany Logo'].join(" ")} width={[props?.data?.meta._mosacademy_testimonial_company_logo_attributes[1], 'px'].join('')} height={[props?.data?.meta._mosacademy_testimonial_company_logo_attributes[2], 'px'].join('')} />
                </div>
                <div className="feedbackText textClrGray fw-normal fs-6 mb-5" dangerouslySetInnerHTML={{__html:props?.data?.content}} />
                </div>
                <div className="reviewerInfo d-flex align-props-center justify-content-between">
                    <div className="d-flex align-props-center gap-3">
                        <div className="pic">
                            <LazyImage className="rounded-circle" width="45px" height="45px" src={props?.data?.image} alt={props?.data?.meta._mosacademy_testimonial_name} />
                        </div>
                        <span className="info">
                            <div className="fs-14 fw-bold text-white mb-1">
                                {props?.data?.meta._mosacademy_testimonial_name}
                            </div>
                            <div className="fs-12 fw-medium textClrGray mb-0" dangerouslySetInnerHTML={{__html:props?.data?.meta._mosacademy_testimonial_designation}} />
                        </span>
                </div>
                <div className="qoute">
